...
chromium.launch=async (...args)=>{
 const browser=await launch(...args); const create=browser.newContext.bind(browser);
    ...
    for (const [label, tar] of [
        ...
        for (const kind of ["minimal", "text-assets", "physics", "blank"]) {
            ...
            const verify = (name: string, pass = true, blocked = false, served = app) => {
                ...
                assert(requests.length > 0, "blocked-network instrument reached actual requests");
                if (blocked)
                    assert(
                        requests.some(
                            (r) => !r.allowed && r.url === "https://outside.invalid/required.bin",
                        ),
