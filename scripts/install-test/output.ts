...
// `shallot verify` is archived; `check` replaces its browser verdicts in this version, so each
// verdict leg below reports itself unavailable while the build and emitted-output audits still run.
...
export async function outputFlow(work: string, candidate: string): Promise<void> {
    ...
    for (const [label, tar] of [["candidate", candidate]] as const) {
        for (const kind of ["minimal", "text-assets", "physics", "blank"]) {
            ...
            exec(`${label}-${kind}-build`, ["bun", "node_modules/.bin/shallot", "build", "."], app);
            ...
            const verify = (name: string) =>
                console.log(`output: ${label}-${kind}-${name} browser verdict: ${UNAVAILABLE}`);
            verify(blank ? "blank-control" : "self-contained");
            if (!blank) {
