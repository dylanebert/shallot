 ...
 *  exercising its own launch — that launch is part of what this flow verifies. */
export function verifyArgs(label: "previous" | "candidate", endpoint: string): string[] {
    const argv = ["verify", ".", "--dist", "--json", "--timeout", "30000"];
...
export async function compatibilityFlow(work: string, candidate: string): Promise<void> {
    ...
    for (const [label, tar] of [
        ...
        // 0.9.5's verify hard-codes `headless: true`, and a headless launch reaches only a software
        // rasterizer on a seat whose real adapter is discrete, which its own display gate then
        // refuses. `--connect` is that CLI's one route to real hardware, and every one of this
        // label's verify legs needs it. The server is launched from the previous install's OWN
        // playwright and shared across the legs: the connect protocol answers a client/server
        // version mismatch with `428 Precondition Required`, and both dirs pin 1.62.1.
        const held: { server: { endpoint: string; stop(): void } | null } = { server: null };
        const transport = async (): Promise<string> =>
            label === "candidate"
                ? ""
                : (held.server ??= await startBrowserServer(app, evidence)).endpoint;
        try {
            ...
            if (label === "previous") {
                assert.equal(
                    diagnostics.length,
                    6,
                    "frozen scaffold type-context failure population",
                );
                assert(
            ...
            const timing = JSON.parse(
                exec(`${label}-timing-migration`, ["bun", "timing.mjs"], app),
            );
            ...
            // 0.9.5's verify hard-codes `headless: true`, and a headless launch reaches only a software
            // rasterizer here, which its own display gate then refuses. `--connect` is its one route to
            // real hardware, so the server is launched from the previous install's OWN playwright: the
            // connect protocol requires an exact client/server version match, and the app pins 1.62.1.
            const verdict = exec(
                `${label}-built-verify`,
                ["bun", "node_modules/.bin/shallot", ...verifyArgs(label, await transport())],
                app,
            );
