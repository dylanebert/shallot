...
// The framework-merge capture fixture (scripts/capture/flows/framework-svelte.pw.ts): a standard
// vite+svelte project the editor opens through its OWN toolchain. Three things resolve only because the
// host merged the project's vite.config — `alias` (resolve.alias $game → lib), `dep` (a bare project dep,
// wgpu-matrix), and the .svelte HUD (the editor's svelte compiles it, the project's own svelte plugin
// deduped out by name). The fields read back through /__api/entities; the HUD renders `define` as its text.
