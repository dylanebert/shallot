...
/** load the cells face (the `cells-face` asset, from the assets cache) the way `extras/text/font.test.ts` does — Node-only
 *  (`readFileSync`), so callers stay confined to this script and its own test, never `ramp.ts`. */
export function loadBrandFont(): Font {
    const bytes = readFileSync(cached("cells-face"));
...
function renderTable(table: readonly RampEntry[]): string {
    ...
    const rows = table
        ...
        .join("\n");
    return `// GENERATED by scripts/generate/generate-ramp.ts — do not hand-edit. Re-run \`bun run
// scripts/generate/generate-ramp.ts\` from the shallot repo root and review the diff.
// The coverage-ordered fill glyph ramp (module doc, scripts/generate/generate-ramp.ts): every printable-ASCII
// candidate minus the directional set (ramp.ts's CELL_DIRECTIONAL_GLYPHS), sorted ascending by measured
// ink coverage against the cells face (the \`cells-face\` asset). \`ramp-table.test.ts\` proves this file still
