...
function build(args: string[], env: Record<string, string> | undefined, targetDir: string): Buffer {
    ...
    return readFileSync(
        resolve(kernelDir, targetDir, "wasm32-unknown-unknown/release/physics_kernel.wasm"),
    );
}
...
// The shared artifact needs nightly (-Zbuild-std). Without it the committed shared kernel stays, and the
// build says so rather than failing a checkout that has only stable Rust.
const nightly = spawnSync("rustup", ["run", "nightly", "rustc", "--version"], { stdio: "ignore" });
if (nightly.status !== 0) {
    console.warn(
        "[build-kernel] no nightly toolchain: kept the committed kernel.shared.wasm.ts (rustup toolchain install nightly to rebuild it)",
    );
} else {
    const shared = build(
        [
