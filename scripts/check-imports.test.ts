...
//
// The arm seeds a temporary violation file in packages/shallot-runtime/src/, runs the script as a
// subprocess, asserts exit 1, and cleans up in finally. The script resolves paths relative to
// import.meta.dir (hardcoded to packages/shallot-runtime/src), so the seed must land in the real tree.
...
const SEED = resolve(
    import.meta.dir,
    "../packages/shallot-runtime/src/engine/_s3_arm_violation.ts",
);
...
test("check-imports — violations exit nonzero (exit 1)", async () => {
    ...
    // through a non-barrel path. check-imports.ts scans packages/shallot-runtime/src for .ts files and
