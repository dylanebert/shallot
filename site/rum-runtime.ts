...
// `define`) — never imported. `packages/shallot-runtime/src/engine/runtime/gpu.ts`'s
...
if (loafSupported) {
    try {
        ...
        // `PerformanceEntry` yet — same shape as `packages/shallot-cli/bin/verify.ts`'s own LoAF observer.
        new PerformanceObserver((list) => {
