...
Voice: instructional, second person, present tense. Confident and plain, a competent peer explaining the thing, not a brochure and not a man page. Prose stays clear and neutral, never flat recitation.

**The bar is mechanized, not left to discipline.** The live site shipped ~80 em dashes past a rule that said "keep them near zero," so the writing bar is a `docs:check` gate (`packages/shallot/scripts/docs-check.ts`), not a guideline that generation pressure erodes. Em dashes in docs prose are zero. The banned AI-tell words and editorial openers live in that script's `BANNED` / `OPENERS` lists (curated for engine vocabulary, so a literal use of `harness` or `navigate` doesn't false-positive), where a contributor sees and edits them; add a word there, not to a rule. A `#doc:code` walkthrough block over three sentences fails the budget class.

- **Drop, don't pad.** A page that can't earn its length is deleted, not filled to look complete. A thin Internals tab loses the tab (`hardening.md`); a page with no real audience loses the page. Shrinking the surface is the fix, never padding it toward a template.
- **Guide pages lead editor-first.** `docs/guide/` is the on-ramp; a guide page leads with the editor or CLI workflow and drops into code only where the editor can't reach (writing a system). Per-module reference pages stay code-first.
...
- **Anchor the unknown to the known.** Introduce a concept by its closest familiar analogue, then state the difference; it lets you skip re-explaining the familiar half. Simple first, complexity builds.
