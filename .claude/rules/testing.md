...
**Where the sweep's cost actually is**, so the cheap-looking levers aren't re-proposed. Measured 2026-08-03 on M-series/Metal: vite server boot 0.09 s, chromium launch 0.22 s, first context+page 0.77 s, each later one 0.28 s. The removable per-scenario fixed cost is the cold module-transform graph (vite re-transforming the engine through the TypeGPU plugin once per OS process) at ~1.1–1.4 s, which batch mode recovers. Everything left is `run()` itself — ~5 s of `outline`'s 6.85 s — so the sweep floor is the scenarios' own frame budgets, not process overhead. Three levers were rejected on that measurement: a `--dist` build (boot was never the cost, and it diverges from the dev path consumers run), parallel worker processes (multiplies the cold transform by N and contends N ways on one GPU, corrupting every timing row), and reusing one page across runs (breaks the one-verdict law and lets module singletons plus device state bleed, to save 0.28 s). Use `shallot verify --timings` to attribute a slow or hung run to a phase before theorizing about one.
