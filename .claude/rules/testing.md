...
Keep AGENTS.md's Verification triggers. CPU precedes display. Zero/unavailable/success differ; `SHALLOT_DISPLAY_REQUIRED=1` refuses unavailable.

## GPU testing

CPU TGSL proves logic; JS mirrors need bit-exact differentials. Default tests may bind adapters but cannot depend on execution except slab repooling. Software-adapter tiers de-risk lifecycle, not correctness. Constants-only preload waits for build tests/probes to own setup. Feature/limit law: `gpu.md`.
...
Cycle order: bump all `check-versions.ts` sites/deferred changelog entries; `bun run scripts/check-versions.ts --release` before pack; RC dogfood; publish engine and scaffold; then merge and tag. Publish/tag together.
