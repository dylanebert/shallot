...
paths:
    ...
    - "packages/shallot/editor/**/*.test.ts"
    - "scripts/capture/**/*.ts"
---
...
A live `State` is reused across hot reloads — the editor swaps a reloaded plugin onto it in place (`ecs.md` "Reload-safety", engine `swap()`). Reload-safety is a standing tier, mechanism-level like CRUD: one test per pattern, not per component or plugin.
...
- **Live HMR — capture flow.** The end-to-end check the engine tests can't reach: `scripts/capture`'s `hot-reload` flow drives the swap-vs-rebuild decision live in four phases — an edit-mode constant edit lands as an in-place swap (runtime counter keeps climbing), the same edit swaps onto the play State, a component-schema edit is rejected by `swap` so the editor rebuilds from the document (new schema live, never wedged), and a throwing reload is quarantined (counter freezes, editor stays responsive) until the next good edit resumes it. Editor tier — display-gated, `bun run capture`.
- **Survive-reload — capture flow.** The other end-to-end consumer: `scripts/capture`'s `survive-reload` flow drives a standalone app through a real `page.reload()` and asserts a runtime value restored and a `warm`-derived entity wasn't doubled. The app composes survival from the core atoms (`serialize`→`sessionStorage` on unload, restore via `run({ scene })` on boot) — no engine flag; the engine stays unopinionated about storage/trigger. It targets the **standalone-app** dev server, not the editor (`FlowDef.target: "app"` → `scripts/capture/app`) — capture now drives both. Display-gated, `bun run capture`.
- **Per-plugin conformance — `bun test` + capture flow.** `tests/conformance.test.ts` runs each roster plugin through two identical build→step→dispose passes against the same module singletons and diffs the observable signature — a double-registering registry or a doubling `warm` spawn goes red. A plugin-toggle sequence (changed-set rebuild) additionally catches a producer toggled off leaving stale registry entries; a seeded non-idempotent fixture pins the harness itself (detail in the file header). The default GPU plugin set rides the same loop live in the editor via the `reload-conformance` capture flow (play/stop ×2 + scene-switch round-trip). Physics/Character/Player can't build on the bun-webgpu adapter — they join at the real-GPU tier when a rebuild-loop gym scenario exists.
...
The editor extends the GPU boundary with one more axis: does the assertion's truth need a real DOM or the engine drawing into the canvas? Same rule — one source of truth per tier, no overlap.

- **Model + view-model — `bun test`.** The Document / Session / command logic *and* the reflection→UI derivation (inspector `schema + traits → sections`, outliner `tree → rows`). The derivation is a pure transform in `lib/` (`svelte.md`), so it tests against synthetic components from the registry with no DOM. Reflection content bugs — a derived field that crashes `readFields` on select, a node that titles "entity" — are view-model bugs, caught here, not at the slow tier. Two property targets live here too: command invertibility (`apply`∘`reverse` = identity, the ProseMirror step contract) and reflection round-trip (`defaults → read → format → parse` is a fixed point per registered component).
- **Behavior + render — Playwright over the real editor (`scripts/capture/`).** Anything whose truth needs the mounted DOM or a real frame: panel presence, selection class, inspector sections (DOM-state), and edit-mode canvas non-blank + selection outline pixels (render). Drive the editor through its control API (`shallot:request` → `state` / `command` / `entities`) — the same seam an agent uses, not a test backdoor. The contract that fails a boot-clean-but-blank editor lives here: **edit mode renders non-background pixels.** Display-gated and single-threaded (port + browser + GPU contention), like `bun bench`. Promote `assert`s into the existing `capture` flows rather than a parallel runner; screenshots are a side artifact of the same session.

**Interaction logic factors inward, like the view-models.** As the editor gains direct manipulation (gizmo drag, command-palette filtering, key routing), each feature's logic is a pure `.ts` transform (screen-delta → transform, query → ranked matches, key → command), tested at the `bun test` tier the same way the reflection derivations are. The `.svelte` handler is a thin call into it. The Playwright tier then verifies only the wiring: each interactive surface gets a flow driving it through real pointer/keyboard events and asserting behavior — a viewport click selects and matches the outliner, a gizmo drag moves the entity, ⌘K opens and filters and runs an action, panels split and collapse. The flow asserts behavior; the screenshot stays the side artifact.

This tier is **expensive** — a real browser + a real GPU, display-gated — so it is **not** in the automated bar (`bun check` / `bun test`). Run it on demand (`bun run capture`) after changes to its domain — the editor UI, the editor↔engine render integration, the engine reload path (`swap` / registry / plugin lifecycle → the `hot-reload` + `reload-conformance` flows), or the capture flows themselves — the same "run after its domain changes" discipline `bun bench` follows for GPU code. This is a gate you run, not a user-only step: it's display-gated but self-terminating (WSL → Windows Chrome, auto-skips when no display). It stays `headless: false` only so the screenshots are viewable.

No mounted-component-without-engine tier. It re-tests Svelte's own reactivity (framework behavior) and the wiring the e2e covers for real, at the cost of a second toolchain.
