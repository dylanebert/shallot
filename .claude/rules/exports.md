...
List import-time effects AND barrels hosting bare effect imports in `sideEffects`, exact paths, no globs. Physics stays opt-in and tree-shakes away: Physics/Physics on the main barrel, inlined physics kernel with no extra install, AVBD only on its subpath. Backend install/uninstall and system anchors stay on `physics/core`, with no solver there. Curate `physics/core` to mirror new engine public symbols but NEVER export shutdown: the shared worker pool is engine-owned. Init and read-only threads introspection remain, not threading knobs.
