...
- **Self-contained.** The canonical project shape: `shallot.json` (scene + plugin enablement) + `public/scenes/*.scene` + plugin modules under `src/`, run via `shallot dev examples/recipes/<entry>/`. No `index.html`, no vite config. Recipes never import from each other or from repo `scripts/` — a small shared shape (a ragdoll build, a ground plane) duplicates rather than coupling entries.
