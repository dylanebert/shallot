// render — one registered scenario carrying many `mode`-selected rows: the structural
// cull/cluster/light/shadow metadata oracles, the shaded-look framebuffer probes, typed variants, the
// glTF import rows, skin-live, transparency and the background/sky backdrops. That is why its coverage
// folds into one `covers` key (`timeouts.ts`) rather than one key per row.
import {
    AmbientLight,
    Backdrop,
    Body,
