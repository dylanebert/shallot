 ...
 * pre-fix `uniformLoad` leaf made the light-cull and BVH binning shaders
...
describe("WGSL 1.0 portability", () => {
    test("no production shader declares a pointer parameter outside function/private space", () => {
        ...
        // (what a no-`subgroups` adapter such as Firefox 155 actually compiles)
        expect(Object.keys(population).length).toBe(3 + 5 + 4);
