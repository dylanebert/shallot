...
**Base floor (every app):** WebGPU 1.0 + `indirect-first-instance` + `bgra8unorm-storage` + `rg11b10ufloat-renderable` (the default HDR offscreen + sear's MSAA color target — halves the resolve bandwidth vs rgba16float). The default renderer + slab substrate need these, so they hold for any Shallot app.

**A floor entry earns its place only by being a `DEFAULT_PLUGINS` need.** The floor gates device acquisition for every app, before any plugin loads, so an entry only an opt-in plugin uses rejects hardware that would have run the scene fine. It belongs on that plugin instead. The three deliberate absentees:

- `timestamp-query` → `ProfilePlugin.features`. Only the profiler creates a query set; every other site passes `Compute.span?.(…)`, undefined and valid without the feature. Required there, so an explicitly added debug plugin fails loud rather than silently reporting no spans.
- texture compression (BC / ETC2 / ASTC) → `GltfPlugin.preferredFeatures`, all three. Only the KTX2 transcode consumes them, and it picks a family from `device.features.has(…)` — which reads **false for a feature the device never requested**, even on hardware that has it. So all three are *preferred* (requested wherever present, never gating). That declaration is the *only* thing making the read true, so it holds only for an app that has `GltfPlugin` in its plugin list: `loadGltf` used standalone requests nothing, and every family reads false. Hence `pickTargets` is **total** (no family → no targets, never a throw) and the real gate is per-image, in `gltf/assets.ts` — a geometry-only or PNG-textured import succeeds on any device, and only a KTX2 image with nowhere to transcode to throws an `UnsupportedError` naming the three families.
- `shader-f16` — it gates the WGSL `f16` *type*, not `pack2x16float` / `unpack2x16float` (core WGSL), so the `f16x4` material mirror binds `vec2<u32>` and unpacks: same 8 bytes, bit-identical, no feature. Half-precision *storage* never needs it; only half-precision *declarations and arithmetic* do. A surface or slab that binds its own `f16` element declares `shader-f16` in its plugin's `features`.

**Plugin-declared features:** a plugin declares what it needs beyond the base floor as `Plugin.features` (required — a `GPUFeatureName[]`) or `Plugin.preferredFeatures` (best-effort). `build()` unions each across the active plugins and passes both to `acquireDevice`, which requests `base ∪ required` (a missing required one throws `UnsupportedError` naming it, before any plugin loads) and adds each preferred only where the adapter has it. The two **preferred** cases: `subgroups`, where only the BVH builder (physics broadphase, accel structures) uses subgroup ops and it has an LDS fallback arm (`createBvh` reads `device.features` to pick), so `AvbdPlugin.preferredFeatures = BVH_FEATURES` (from `bvh/core`) — a `subgroups`-less device (WebKit) still loads physics, on the slower LDS arm; and the glTF importer's three compression families (above). A physics-free, glTF-free app requests none of them.
