...
// (`transitional/skin`). The runtime-posed twin of the VAT skin surface (skin.ts) — same material `shadePbr` path,
...
// is `transitional/skin`; only the material composition lives here. A producer that poses a rig by hand needs
...
// substrate's three (`skin` / `skinData` / `skinParams`, transitional/skin) + the shared material palette + the
...
// read to resolution time (see standard/rendering/pipelines.ts's `typedVaryingVs` for the same fix in full).
 ...
 * same material `shadePbr` path + alpha split, over the live joint palette (`LiveSkin`, `transitional/skin`) where
