...
// (`transitional/gltf` registers the `skin-live` PBR trio over {@link LIVE_SKIN_VS}); this file owns the buffer,
 ...
 *  object, so the traits live here once rather than diverging per plugin. */
 ...
 * with whatever material path it wants — `transitional/gltf`'s `skin-live` PBR trio (registered by `GltfPlugin`)
