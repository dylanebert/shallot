...
export interface BenchmarkGpuStats {
     ...
     *  deterministic *untimed* cost (`#drawIndexedIndirect × INDIRECT_FLOOR_US`), invisible to
...
export interface BenchmarkCpuStats {
     ...
     *  the 100µs `performance.now` quantization makes min biased) */
...
export interface BenchmarkFrameStats {
     ...
     *  Reads ~1 higher under the profiler than in production (the resolve submit). */
 ...
 *  fence wait, not a pass time. Calibrated 2026-06-14 via an
