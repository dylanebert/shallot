...
type BruteTriangle = {
    index: number;
    vertices: ReturnType<typeof getHeightFieldTriangle>["vertices"];
};
...
function bruteForceShapeCast(
    ...
    for (const triangle of triangles) {
        ...
        if (out.hit && out.fraction < bestFraction) {
            ...
            best = out;
            best.triangleIndex = triangle.index;
        }
    }
    return best;
}
...
function assertShapeCastGridMatches(deltas: readonly Vec3[]): void {
    const hf = createWave(10, 10, { x: 2, y: 1.5, z: 2 }, 0.1, 0.03333, false);
    const radii = [0.15, 0.4, 0.9];
    const triangles: BruteTriangle[] = [];
    for (let t = 0; t < hf.flags.length; ++t) {
        if (hf.materialIndices[t >> 1] !== HEIGHT_FIELD_HOLE)
            triangles.push({ index: t, vertices: getHeightFieldTriangle(hf, t).vertices });
    }
...
check(
    ...
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(0, 1)),
);
...
check(
    ...
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(1, 2)),
);

check(
    "height field shape cast grid walk matches lateral casts",
    {
        claim: "the grid walk in shapeCastHeightField would disagree with brute-force lateral casts over every wave height field triangle",
    },
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(2, 3)),
);
...
check(
    ...
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(3, 4)),
);
...
check(
    ...
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(4, 5)),
);
...
check(
    ...
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(5, 6)),
);

check(
    "height field shape cast grid walk matches negative-z diagonal casts",
    {
        claim: "the grid walk in shapeCastHeightField would disagree with brute-force negative-z diagonal casts over every wave height field triangle",
    },
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(6, 7)),
);
...
check(
    ...
    () => assertShapeCastGridMatches(shapeCastDeltas.slice(7)),
);
