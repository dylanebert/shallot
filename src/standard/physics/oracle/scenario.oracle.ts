...
const BENCHMARK_ROSTER = ["bench-pyramid", "bench-many-pyramids", "bench-joint-grid", "bench-washer", "bench-large-world", "bench-trees", "bench-junkyard", "bench-rain", "drift"];
function compareFamily(roster: string[]): void {
    const { corpus, digest } = loadScenarioCorpus();
    const cumulative = [...FOUNDATION_ROSTER, ...JOINT_ROSTER, ...SURFACE_ROSTER, ...COMPOUND_SENSOR_ROSTER, ...BENCHMARK_ROSTER];
    ...
    for (const scenario of selected) {
        ...
        compareOutputs(runScenario(scenario, digest), runLegacyScenario(scenario.name, world?.enableSleep === true, world?.enableContinuous === true, scenario.stepCount));
    }
}
...
check("O5e Shallot benchmark command interpreter migration", { claim: "box3d-scenario-migration-benchmarks", size: "integration" }, () => compareFamily(BENCHMARK_ROSTER));

check("O5e Shallot benchmark timing mutations", { claim: "box3d-scenario-migration-benchmark-timing", size: "integration" }, () => {
    const { corpus, digest } = loadScenarioCorpus();
    const moveCommands = (name: string, predicate: (command: (typeof corpus.scenarios[number])["commands"][number]) => boolean, count: number, afterStep: string): void => {
        ...
        const stepIndex = scenario.commands.findIndex((command) => command.id === afterStep && command.op === "step");
        if (index < 0 || stepIndex < 0 || index > stepIndex || index + count > scenario.commands.length) throw new Error(`${name} timing command is not before its scheduled step`);
        const moved = scenario.commands.splice(index, count);
        const newStepIndex = scenario.commands.findIndex((command) => command.id === afterStep && command.op === "step");
        scenario.commands.splice(newStepIndex + 1, 0, ...moved);
        const baseline = runScenario(corpus.scenarios.find((item) => item.name === name)!, digest);
        const changed = runScenario(scenario, digest);
        if (JSON.stringify(baseline.observations) === JSON.stringify(changed.observations) || JSON.stringify(baseline.hashes) === JSON.stringify(changed.hashes)) throw new Error(`${name} timing mutation did not change the replay`);
    };
    moveCommands("bench-large-world", (command) => command.op === "body.spawn" && command.id === "b144", 2, "step-005");
    moveCommands("bench-junkyard", (command) => command.op === "body.target-transform" && command.id === "target-000", 1, "step-000");
});
...
check("O5a command receipt and adversarial gates", { claim: "box3d-scenario-command-corpus", size: "integration" }, () => {
    ...
    const bulletBody = ccdBullet.commands.find((command) => command.op === "body.create" && command.id === "b2");
    ...
    if (JSON.stringify(bulletBaseline.observations) === JSON.stringify(bulletChanged.observations) || JSON.stringify(bulletBaseline.hashes) === JSON.stringify(bulletChanged.hashes)) throw new Error("ccd-bullet x-velocity mutation did not reach the TypeScript adapter");
    ...
    const missingGeometry = structuredClone(corpus.scenarios.find((scenario) => scenario.name === "mesh-box"));
    if (!missingGeometry) throw new Error("mesh-box geometry target is missing");
    missingGeometry.commands = missingGeometry.commands.filter((command) => command.op !== "resource.mesh");
    try { runScenario(missingGeometry, digest); throw new Error("missing geometry unexpectedly succeeded"); } catch (error) { if (!(error instanceof Error) || !error.message.includes("unknown mesh")) throw error; }
    const missingReference = structuredClone(corpus.scenarios.find((scenario) => scenario.name === "height-sphere"));
