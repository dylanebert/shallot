...
// then one sampled cycle whose event frames count every byte, the steady windows after it, and a second
// cycle sampled live-only: after despawn and a full collection, what the cycle allocated and still holds.
async function transition() {
    ...
    collect();
    const { profile } = await session.post("HeapProfiler.stopSampling");
    return { spawn, despawn, windows, survivors: sites(profile) };
}
