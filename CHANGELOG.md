...
- **known red** — the blank-render report on Apple Metal was a fixture defect, not an engine defect: the install gate's fixture never framed its camera on the scene, so the arm captured the cleared background. Framing both fixture scenes as the scaffold does retired the red — before, 4/4 runs red (6/8 arms); after, 3/3 runs green (6/6 arms) — and 6/6 green excludes the before-regime but not a residual ~10% flake, so the honest claim is that the red no longer reproduces.
