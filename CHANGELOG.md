...
- **showcase** — `examples/showcase/roads` is a new procedural-terrain exhibit, shipped in the tarball with the other showcases: a seeded-FBM heightfield with a road cut into an earthwork corridor, a 2D overlay rasterizer for the network, and an interactive reseed. It runs on a real GPU under its own device gate.
...
- **internals** — six audit passes over the package, none of them reachable from a named subpath. Behavioral fixes: `f16` encoding saturates a finite overflow to infinity, matrix `invert` zeroes its output on a singular input, the fixed-step scheduler caps steps on the post-scale accumulator, `State.create` rolls back an over-capacity entity, `Player`/`Text` dispose by identity, and off-canvas pointer motion still tracks. Roughly thirty zero-consumer exports are deleted or made private, duplicated stride constants are re-derived from their schemas, and GPU debug labels read `shallot`.
