...
The verification vehicle is the **shipped gate**: `shallot verify [dir]` boots a project in a headless browser, waits to render (or drives the `window.__harness` a project installs), reads a pass/fail Verdict, exits 0/nonzero. There is no repo-private harness tier: `bun bench` and `bun run flows` are thin wrappers over that CLI. A numeric flag parser rejects a non-numeric or empty value with a throw or nonzero exit, never a default — `Number("")` is `0` not `NaN`, so a typo silently no-ops or flows `NaN` without `v.trim() === ""` before `Number()`.
