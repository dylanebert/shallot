...
// The pure-math constants and the falloff derivation live in `flatten-math.ts` so modules that only
// need the math (not the GPU kernel) can import without pulling in `@dylanebert/shallot`'s device-bound
// module graph — `overlay/network.ts` and the Playwright test that imports it. Imported here (not just
// re-exported) so the TGSL `tgpu.fn` kernels below can resolve `FLAT_CORE_MARGIN` from this module's own
// scope, and re-exported so existing imports from `flatten.ts` (bun-test files, `flatness.ts`) are
// unchanged.
import { computeFalloff, FLAT_CORE_MARGIN, SIDE_SLOPE_LIMIT } from "./flatten-math";
import { heightAt, makePermutation } from "./noise";
import { buildPolylineProfile, heightAtCpu, PROFILE_STEP } from "./profile";

export { computeFalloff, FLAT_CORE_MARGIN, SIDE_SLOPE_LIMIT };
