...
//
// The drag is what picks the route: optimizing a road on an axis the interaction abandons at the first
// grab is a cost optimizer maintained for a state the user drags away from. The boot road is simply a
// standard road: one fixed chord, no placement search and no seeded draw. `generateNetwork` takes no
// seed; `regenerate(seed)`'s seed drives the terrain's noise permutation alone (`terrain/terrain.ts`),
// never the road's position.
...
// this clamps the endpoint to the closest point holding the floor. Its reason is the artifact
 ...
 *  machinery. No seed: the road's position is authored, not drawn — a person's drag is the only thing
 *  that ever moves it. */
 ...
 * "nearest to origin" search over a set that's always size 1. The on-road point is that midpoint's
 * nearest grid vertex; the off-road
