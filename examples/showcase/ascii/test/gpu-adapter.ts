...
// Shared real-GPU adapter probe: every ascii Playwright spec needs the same "skip on a software adapter"
// check before waiting on the app's boot hook. Not itself a `*.playwright.ts`, so Playwright's own
// testMatch never picks it up as a test file.
