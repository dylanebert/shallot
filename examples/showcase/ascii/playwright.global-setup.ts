...
// ascii's gate today, so no live failure, but a future Dawn-unsafe-API use would only hit it over the
// bridge path.
const REPO_ROOT = resolve(import.meta.dirname, "../../..");
const BRIDGE_CONNECT = resolve(REPO_ROOT, "scripts/wsl-bridge-connect.ts");
export const ENDPOINT_FILE = resolve(
    import.meta.dirname,
    "node_modules/.cache/ascii-bridge-endpoint.json",
);
...
export default async function globalSetup(): Promise<(() => Promise<void>) | void> {
    ...
    try {
        ...
        child.stdin?.end();
        console.log(
            `ascii gate: bridge unavailable (${e instanceof Error ? e.message : e}), falling back to local adapter`,
        );
        return;
    }
    ...
    if ("skip" in handshake) {
        console.log(
            `ascii gate: bridge unavailable (${handshake.skip}), falling back to local adapter`,
        );
        child.stdin?.end();
        return;
    }
    ...
    try {
        ...
        child.stdin?.end();
        console.log(
            `ascii gate: bridge unavailable (couldn't write the endpoint handoff: ${e instanceof Error ? e.message : e}), falling back to local adapter`,
        );
        return;
    }
