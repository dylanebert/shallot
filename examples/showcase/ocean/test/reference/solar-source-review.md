...
The circumsolar field is compactly bounded at `2.5°`, the recommended pyrheliometer acceptance half-angle in ISO 9060:2018, §5.4.4 and table 2: <https://www.iso.org/standard/67464.html>. That aperture is an instrument-grounded boundary for direct plus circumsolar irradiance, not an atmosphere simulation. A smoothstep decreases from the disk edge to zero at that boundary. The display-referred exposure multipliers are explicitly declared as `16` for the existing disk and `1.5` for the circumsolar field, so the neighborhood peaks at `9.375%` of the disk-center scale before the shared color and scene exposure; no bloom or post-processing is implied.
