...
//
// Known, accepted gaps (adversarial review, 2026-08-18, ported from `roads/playwright.global-setup.ts`):
// this is a second caller into `wsl-bridge.ts`'s single-session bridge alongside `scripts/verify.ts`'s
// wrappers — running this gate concurrently with `bun bench`/`bun run flows` races the same host staging
// dir, same as any two bridge callers would (the bridge itself, not this diff, owns the one-at-a-time
// constraint; a lock is a separate unit). And the bridge-launched browser only carries `wsl-bridge.ts`'s
// own hardcoded launch flags, not this config's `--enable-dawn-features=allow_unsafe_apis`
// (`connectOptions` makes `launchOptions` inert) — unused by today's demo checks, so no live failure, but
// a future Dawn-unsafe-API use here would only hit it over the bridge path.
const REPO_ROOT = resolve(import.meta.dirname, "../../..");
const BRIDGE_CONNECT = resolve(REPO_ROOT, "scripts/wsl-bridge-connect.ts");
export const ENDPOINT_FILE = resolve(
    import.meta.dirname,
    "node_modules/.cache/visualization-bridge-endpoint.json",
);
...
export default async function globalSetup(): Promise<(() => Promise<void>) | void> {
    ...
    try {
        ...
        child.stdin?.end();
        console.log(
            `visualization gate: bridge unavailable (${e instanceof Error ? e.message : e}), falling back to local adapter`,
        );
        return;
    }
    ...
    if ("skip" in handshake) {
        console.log(
            `visualization gate: bridge unavailable (${handshake.skip}), falling back to local adapter`,
        );
        child.stdin?.end();
        return;
    }
    ...
    try {
        ...
        child.stdin?.end();
        console.log(
            `visualization gate: bridge unavailable (couldn't write the endpoint handoff: ${e instanceof Error ? e.message : e}), falling back to local adapter`,
        );
        return;
    }
