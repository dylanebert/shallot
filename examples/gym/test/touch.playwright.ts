...
// S4's touch verification gate (spec: `shallot-mobile-controls`). Drives real CDP touch against gym's
// `orbit-touch` scenario (`../src/scenarios/orbit-touch.ts`: an Orbit camera targeting a box) and reads
// the pose back through `window.__orbitPose()`, which folds the camera's live `Transform` together with
// the public `Orbit` component's own target pose (yaw/pitch/distance/pan) — the direct-component half is
// what lets each assertion isolate its gesture's own quantity: the Transform read is the *smoothed*
// rendered pose (`smoothLerp`, extras/orbit) and lags a live drag by several frames, while
// `Orbit.yaw`/`distance`/`pan` are the immediate per-frame target the drag/pinch/pan math writes. This is
// a driver-level gate — an external driver reading the scenario's state, the same role `bun bench` or a
// human plays for any other scenario, so the scenario contract's environment-unawareness (which binds
// `assert`, never a driver) is untouched.
//
...
// re-projection noise the camera's Transform pos/quat can carry between frames, well below a single real
...
test("orbit touch gate — pinch/drag/pan isolation, 2→1 finger transition (real touch)", async ({
    ...
    await page.goto("/?scenario=orbit-touch");
