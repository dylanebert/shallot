...
//
// Outside the default `bun run test` (the `test` script's explicit paths don't include `examples/gym/src`) — run via `bun test ./examples/gym/src`, or a single file:
//   bun test ./examples/gym/src/physics-overlay.test.ts

import { expect, test } from "bun:test";
import { init, World } from "@dylanebert/shallot/physics/core";
import { buildJointBreak, renderJointBreak } from "./physics-joint-break";
import type { Overlay } from "./physics-overlay";
