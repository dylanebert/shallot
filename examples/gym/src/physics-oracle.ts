// The gold-match oracle for the physics sample host: rebuild a sample's world from the escape-hatch build,
// step it N times at knob defaults, and compare each step's `hashWorldState` against the committed gold
// trajectory (tests/physics/samples/<slug>.json). Both sides are the same engine, so a match is bit-exact
...
/** authors a sample's scene into a live `World`, reproducing the upstream sample's `build()` — the one
...
/** the per-step hook run just before each `world.step`, reproducing the upstream sample's `update(dt)`
 ...
 *  `scripts/physics/gen-sample-golds.ts`). The host consumes the imported JSON as this shape. */
