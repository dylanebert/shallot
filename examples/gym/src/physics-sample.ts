// The gym physics-sample host: the one-time layer every ported upstream sample runs on. A scenario hands
...
//      walk drives it every frame: solid bodies render as shallot-native instanced Parts (`physics-solids.ts`,
...
import { beginGrab, driveGrab, endGrab, type Grab, updateGrab } from "./physics-grab";
import {
    goldParams,
    type OracleResult,
    runOracle,
...
import { type OverlayLayer, overlayLayer, type SampleRender } from "./physics-overlay";
import { type ProbeContext, runProbe } from "./physics-probe";
import {
    collectSolids,
    meshKey,
    registerSolids,
...
import { installWatch } from "./physics-watch";
...
/**
 * Build a gym {@link Scenario} that verifies a ported upstream sample against its gold and renders it with
    ...
    return {
        ...
        async build(canvas: HTMLCanvasElement, params: Params) {
            ...
            const stepper: System = {
                name: `physics-sample-step:${gold.slug}`,
                group: "fixed",
                update() {
            ...
            const draw: System = {
                name: `physics-sample-draw:${gold.slug}`,
                group: "simulation",
                update() {
                    solidLayer.begin();
...
export function solidPool(
    ...
    const adapter: DebugDraw = {
        ...
        // pack's frustum cull owns on-screen visibility, not this walk (`physics-solids.ts` TOTAL_DRAW_BOUNDS).
