// Floor-vanish instrument for the physics sample host. Two surfaces, one
// shared readback core:
//
//   • the passive page hooks — `window.__physicsProbe` (a drawArgs readback + a NaN/Inf scan over body poses
//     AND Part instance transforms), `window.__physicsAim` (a grabbable dynamic body's client pixel), and
//     `window.__physicsInject` (a self-test poison for the red-first proof). Zero per-frame cost: they run
//     only when a driver calls them. The trusted-input Playwright driver (`scripts/physics-repro-driver.mjs`)
...
//     non-finite, latches a full dump onto `window.__physicsFloorDump` and console.error's it. So a by-hand
...
// readback, which stalls the pipeline and would skew the profiler frame timing that `bun run scripts/bench-physics.ts`
...
// (except `__physicsInject`, which exists solely to prove the watcher fires and is never called in a real run).
...
import type { ProbeContext } from "./physics-probe";
import { cameraPose, worldToScreen } from "./physics-project";
...
// catches it on both sides. Never called in a real run; the driver calls it once through `__physicsInject`.
...
declare global {
    interface Window {
        __physicsProbe?: () => Promise<Snapshot>;
        __physicsAim?: () => { clientX: number; clientY: number } | null;
        __physicsInject?: (kind: string) => boolean;
        __physicsDeep?: () => Promise<DeepReadback>;
        __physicsFloorDump?: FloorDump;
        // F1′ recipe instrument (spec 6b/F1′)
        __physicsAimAt?: (wx: number, wy: number, wz: number) => ReturnType<typeof aimAt>;
        __physicsProject?: (pts: [number, number, number][]) => ScreenPoint[];
        __physicsStatics?: () => number[][];
        __physicsCam?: () => ReturnType<typeof camInfo>;
        __physicsGrab?: () => ReturnType<NonNullable<ProbeContext["grab"]>>;
        // F2′ render-layer readback (spec 6b/F2′)
        __physicsRender?: () => Promise<RenderState>;
        __physicsVelPeak?: (reset?: boolean) => {
            maxVel: number;
            maxAngVel: number;
            nonFiniteVel: number;
        };
        // F3′ world-level static-set invariant (spec 6b/F3′)
        __physicsStaticBreach?: () => { statics: StaticState[]; missing: number[] | null };
    }
}
...
export function installWatch(ctx: ProbeContext): void {
    ...
    w.__physicsProbe = () => snapshot(ctx);
    w.__physicsAim = () => {
        const t = findTarget(ctx);
        return t ? { clientX: t.clientX, clientY: t.clientY } : null;
    };
    w.__physicsInject = (kind: string) => inject(ctx, kind);
    w.__physicsDeep = () => deepProbe(ctx);
    w.__physicsAimAt = (wx, wy, wz) => aimAt(ctx, wx, wy, wz);
    w.__physicsProject = (pts) => projectPoints(ctx, pts);
    w.__physicsStatics = () => statics(ctx);
    w.__physicsCam = () => camInfo(ctx);
    w.__physicsGrab = () => (ctx.grab ? ctx.grab() : null);
    ...
    w.__physicsStaticBreach = () => staticInvariant(ctx, registeredStatics);
    w.__physicsRender = () => renderState(ctx);
    w.__physicsVelPeak = (reset?: boolean) => {
        const r = { ...velPeak };
        if (reset) velPeak = { maxVel: 0, maxAngVel: 0, nonFiniteVel: 0 };
        return r;
    };
    ctx.state.onDispose(() => {
        w.__physicsProbe = undefined;
        w.__physicsAim = undefined;
        w.__physicsInject = undefined;
        w.__physicsDeep = undefined;
        w.__physicsFloorDump = undefined;
        w.__physicsAimAt = undefined;
        w.__physicsProject = undefined;
        w.__physicsStatics = undefined;
        w.__physicsCam = undefined;
        w.__physicsGrab = undefined;
        w.__physicsRender = undefined;
        w.__physicsVelPeak = undefined;
        w.__physicsStaticBreach = undefined;
    });
    ...
    ctx.state.addSystem({
        ...
        update() {
            ...
            if (breach && !dumped) {
                ...
                w.__physicsFloorDump = dump;
                console.error(
                    `[physics-floor-watch] BREACH frame ${frame}: ${breach.kind} — drawing ${breach.drawing} of ${breach.meshes} mesh(es), ${nonFinite.length} non-finite ${JSON.stringify(nonFinite.slice(0, 4))}`,
                );
                ...
                deepProbe(ctx)
                    .then((deep) => {
                        dump.deep = deep;
                        console.error(
                            `[physics-floor-watch] deep: frustumFinite=${deep.frustumFinite} extreme=${deep.frustumExtreme.toExponential(2)} flung=${deep.flung.length} mismatch=${deep.mismatch.length} ${JSON.stringify(deep.mismatch.slice(0, 4))}`,
                        );
                    })
                    .catch(() => {});
            }
