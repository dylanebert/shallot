...
// the full physics gold-replay population, moved out of the default suite because it costs
// ~5.2 s of test time there against the 5 s per-file cap. The cost is the population —
// 38 ported upstream samples, each with a committed gold trajectory replayed bit-exact
...
// and did not shrink — the default suite keeps `physics-golds.test.ts` as a sentinel over
// the first 5 entries (16 jobs) against the same frozen gold fixtures.
//
// Run by path, one file per invocation:
//   bun test ./examples/gym/src/physics-golds.tier.ts     (from the shallot root)
//
...
// imports `physics-gold-pool.ts` and `physics-registry.ts`; `physics-registry.ts` imports all
// 38 `physics-*.ts` build/update modules and 38 committed gold JSON files under
// `src/standard/physics/samples/`; `physics-gold-pool.ts` imports only
// `physics-registry.ts` (for the `GoldEntry` type) and resolves `physics-gold-runner.ts` at
// runtime; `physics-gold-runner.ts` imports `@dylanebert/shallot/physics/core`,
// `physics-oracle.ts`, and `./physics-registry`. The cone is wider than the set of modules a gold verdict is a function
...
import { type Job, type JobResult, jobsFor, runJobs } from "./physics-gold-pool";
import { goldRegistry } from "./physics-registry";
