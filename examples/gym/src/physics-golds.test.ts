// Sentinel for the physics gold-replay tier (`physics-golds.tier.ts`): the first
...
// (`physics-gold-pool.ts`), assert each exits clean.
//
// One world per child is correctness, not tidiness. The physics kernel is a process-wide wasm singleton
...
// recipe (`scripts/physics/gen-sample-golds.ts`) — removes both: registry order cannot affect any verdict
...
//   bun test ./examples/gym/src/physics-golds.test.ts
