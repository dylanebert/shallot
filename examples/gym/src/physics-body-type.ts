// The upstream `BodyTypes` sample (`samples/src/samples/bodies.ts`) reproduced near-verbatim through the
...
// the seam `physics-pilot.test.ts` proves red/green against this exact gold.
//
// The sample's live `setType` re-typing (its knob is `live: true`) is out of scope: the host renders every
// select knob as a `rebuild` knob (`physics-sample.ts` `knobParams`), so a type change reloads the scene
...
import type { SampleParams, SampleUpdate } from "./physics-oracle";
