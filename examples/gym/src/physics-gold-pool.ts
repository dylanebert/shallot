// Shared child-process pool for the physics gold-replay tests. Both the default-suite sentinel
// (`physics-golds.test.ts`) and the by-path tier (`physics-golds.tier.ts`) drive the same runner
// (`physics-gold-runner.ts`) through this pool — the only difference is which registry entries each
...
import type { GoldEntry } from "./physics-registry";

const runner = resolve(import.meta.dir, "physics-gold-runner.ts");
