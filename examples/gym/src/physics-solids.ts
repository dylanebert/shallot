// Native solid rendering for the physics sample host, derived entirely from the world's own shape data.
//
...
// The `world.draw` gizmo pass (joints, contact points) stays on top as lines (`physics-sample.ts`).
//
...
// the derivation's, so both the discovery and the per-frame walks (`physics-sample.ts`) use these effectively
...
export function registerSolids(defs: GeomDef[]): Map<string, number> {
    ...
    const vertices = Compute.root
        .createBuffer(d.arrayOf(d.vec4u, q.main.length / 4), storage("physics-solid-main", q.main))
        .$usage("storage");
    const position = Compute.root
        .createBuffer(
            d.arrayOf(d.vec2u, q.position.length / 2),
            storage("physics-solid-pos", q.position),
        )
        .$usage("storage");
    const quant = Compute.root
        .createBuffer(
            d.arrayOf(MeshQuant, q.quant.length / 12),
            storage("physics-solid-quant", q.quant),
        )
        .$usage("storage");
    const indices = Compute.root
        .createBuffer(
            d.arrayOf(d.u32, packed.indices.length),
            storage("physics-solid-idx", packed.indices, true),
        )
        .$usage("storage", "index");
