// One gold-test job — a single physics world — run in an isolated child process. The
// spawner (`physics-gold-pool.ts`, shared by both the sentinel `physics-golds.test.ts` and the
// tier `physics-golds.tier.ts`) launches this once per job as
...
//                         gold-checked)
//
// One world per process is correctness, not tidiness. The physics kernel is a process-wide wasm singleton
...
// mint used (`scripts/physics/gen-sample-golds.ts`), making registry order irrelevant to every verdict.
//
...
import { goldParams, runOracle } from "./physics-oracle";
import { type GoldEntry, goldRegistry } from "./physics-registry";
...
if (!slug || slug.startsWith("--")) fail("[physics-gold-runner] missing --slug <slug>");
const entry = goldRegistry.find((e) => e.slug === slug);
if (!entry) fail(`[physics-gold-runner] no registry entry for slug ${slug}`);
