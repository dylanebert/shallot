// The interaction + visual standing probe for the physics sample host. Two
...
// verify wrapper (`scripts/bench-physics.ts`'s shape) synthesizes the events in-page. They are real `PointerEvent`
...
import type { OverlayLayer } from "./physics-overlay";
import { worldToScreen } from "./physics-project";

/** what the physics host hands the probe: the live world + camera + canvas, enough to project, aim, and
...
// distinct solid geometries in the world, keyed exactly as the solid layer dedupes them (`physics-solids.ts`
...
// whose `instanceCount` (lane 1 of the 20-byte DrawIndexedIndirect stride) is nonzero. The physics host runs
