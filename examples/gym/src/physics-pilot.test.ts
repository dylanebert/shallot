...
import { beginGrab, driveGrab, endGrab, updateGrab } from "./physics-grab";
import { runOracle, type SampleBuild, type SampleGold } from "./physics-oracle";
...
test("the per-step update seam replays a sample whose behavior lives in update()", async () => {
    ...
    // `updateBodyType` (`physics-body-type.ts`) are the real stage-4 port this seam proof now replays.
