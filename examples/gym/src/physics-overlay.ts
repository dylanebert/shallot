// The per-sample overlay channel for the physics sample host: the demonstration layer a upstream sample's
...
//
// The DrawApi surface mirrors upstream's `samples/src/gfx/draw.ts`, trimmed to what the corpus actually
...
// projects to positioned HTML labels over the canvas (`physics-project.ts`); `text` (the sample's `drawText`)
...
import type { SampleParams } from "./physics-oracle";
import { cameraPose, worldToScreen } from "./physics-project";

/** the immediate-draw surface a sample's `render()` calls — the corpus subset of upstream's `DrawApi`, plus
...
// from bottom pole, down the tube, to top pole), matching `physics-solids.ts`'s solid-mesh parametrization.
...
// world center of a solidSphere/solidCapsule local point under `xf` (matches upstream `pushSphere`).
