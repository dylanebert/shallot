...
//
// A system is a function that runs each frame. `state.query([Spin, Transform])` returns every entity that
// has both components, and the loop turns each one by its own `speed`. Derive the angle from
// `state.time.elapsed` instead of adding to a stored value each frame, so it stays correct after a hot
// reload. `group: "simulation"` runs it in play mode, not while editing.
// #region system
export const spin = {
    name: "spin",
    group: "simulation",
...
//
// A marker is a component with no fields — its presence is the whole message. Query for entities that have
