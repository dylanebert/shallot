...
// Vendored from the meshoptimizer npm package 0.22.0 (meshopt_decoder.module.js), the same treatment as the
// Draco / Basis prebuilts beside it: shallot's only runtime deps are typegpu + the Vite/node environment, so
// a codec that ships to consumers is checked in rather than resolved. Self-contained — the wasm is inline
// base64, so there is no sidecar asset and no `import.meta.url` resolution. Do not edit; re-vendor to bump.
