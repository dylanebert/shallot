...
//
// In the default `bun run test`, or alone:
//   bun test ./packages/shallot-tumble/tests/tumble-solids.test.ts

import { expect, test } from "bun:test";
import { Color, capacity, Part, Slab, State, Transform } from "@dylanebert/shallot";
...
import { collectSolids, solidPool } from "./tumble-solids";
...
test("the solid layer derives every world body, even one dragged far from the origin", async () => {
    ...
    register("Part", Part);
    const saved = capacity;
    let ownedState: State | undefined;
    let ownedWorld: World | undefined;
    try {
        const state = (ownedState = new State({ capacity: 1024 }));
        Slab.collect(); // allocate the slab CPU columns at the now-fixed capacity (SlabPlugin's job in a real build)

        const world = (ownedWorld = new World({ gravity: { x: 0, y: -10, z: 0 } }));
        ...
        try {
            ...
            try {
                ...
                new State({ capacity: saved }).dispose();
            }
        }
    }
});
