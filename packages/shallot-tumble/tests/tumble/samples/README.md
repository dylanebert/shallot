...
```
bun run scripts/gen-tumble-sample-golds.ts     # from packages/shallot-tumble
```
...
commit above is historical provenance. `packages/shallot-tumble/scripts/gen-tumble-sample-golds.ts` is kept as the mint's recipe
