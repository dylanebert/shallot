...
- **Unit tests** (`bun test`) — fast, hardware-invariant logic: structure, layout, math errors. **A default-suite one binds no GPU device, bar the exception testing.md names** — software adapters flake. A by-path `.tier.ts` probe binds the preloaded adapter: closed-form bands only, never a correctness oracle.
