...
- **Unit tests** (`bun test`) — fast, hardware-invariant logic: structure, layout, math errors. **Never bind a GPU device in a default-suite unit test** — software adapters flake by construction. A measured by-path `.tier.ts` probe binds the preloaded adapter: closed-form bands only, never a correctness oracle.
