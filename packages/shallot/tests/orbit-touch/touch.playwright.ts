...
// The touch verification gate (spec: `shallot-mobile-controls`). Drives real CDP touch against this
// directory's own fixture app (`src/main.ts`: an Orbit camera targeting a box) and reads the pose back
// through `window.__orbitPose()`, which folds the camera's live `Transform` together with
...
// a driver-level gate — an external driver reading the fixture's state, the same role `bun bench` or a
// human plays for any other page.
//
...
// — display-gated (`./playwright.global-setup.ts`), never part of the default `bun run test` sweep.
