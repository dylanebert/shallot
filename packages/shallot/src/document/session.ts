...
export const ReadbackSystem: System = {
    ...
    update(state: State) {
        ...
        // (a `warm` spawn / a system's add), and the document is the authoring truth: a host's
        // add-component flow writes the doc attr first (via `doc.addAttr`), so a legitimately-added
...
/**
 * the document↔engine bridge: maps document {@link Node}s to live entity ids and applies document commands
 * to the running {@link State}. It owns the reflection reach (getComponent / setFieldValue) so a live
 * authoring host stays out of the ECS internals. A host's live field preview, component attach/detach,
...
export class Session {
    ...
    /**
     * apply a partial field map to the live ECS — a host's per-field live preview during a drag /
     * input gesture. Keys are field names (`intensity`) or dotted Pair/Quad lanes (`pos.x`), matching what
     * a field editor emits; string values resolve through the trait parser (`@name` refs, named enums). The
     * reflection reach (getComponent / setFieldValue) lives here so the document↔engine seam stays in Session.
     */
