...
function fixture(required: boolean, gap: boolean) {
    ...
    // The manifest build loads the project's real configuration before emitting web/native artifacts.
    // This terminal fixture proves that boundary only, never a cargo build or platform launch.
    writeFileSync(
...
function commandReceipt(
    ...
    const result = Bun.spawnSync(
        ...
        { cwd: project.dir, env: process.env, timeout: 4000 },
    );
...
// This fixed composition check complements the dynamic project receipt, which does not observe
// native output/options. It inspects only these CLI files; it is not a command resolver or runner.
...
function imported(tree: ReturnType<typeof ast>, name: string, from: string): string {
    ...
    // A same-spelled local would not be the imported binding. These production files need no such
    // shadowing, so refuse it rather than guessing which declaration a call resolves to.
...
for (const command of ["build", "run"] as const) {
    test(`CLI ${command} dispatch binding preserves project and options`, () => {
        ...
        // Scan the whole command branch, not merely a surviving matching call's argument text.
        expect(
